            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center text-white uppercase">
                ...
                <div class="!text-3xl ">
                    <x-breadcrumb :breadcrumbs="[
                        ['name' => 'Beranda', 'url' => '/'],
                        ['name' => 'Profil Guru', 'url' => '/teacher' . '/' . $teacher->slug],
                    ]"></x-breadcrumb>
                </div>
            </div>
        </div>
                    ...
                    @if ($teacher)
                        <div class="font-jakarta">
                            <div class="flex items-center font-jakarta ">
                                ...
                                <span class="ml-2 text-xs text-gray-600">{{ $teacher->created_at->format('d F Y') }}
                                    ({{ $teacher->created_at->locale('id')->diffForHumans() }})
                                </span>
                            ...
                            <div class="relative w-1/3 h-auto mt-2 mb-5 mr-5 overflow-hidden lg:mb-0 float-start"
                                ...
                                <img src="{{ asset('storage/' . $teacher->image) }}" alt="{{ $teacher->title }}"
                    ...
                    @else
                        <div
                            ...
                            <div class="text-center">
                                ...
                                <h2 class="mb-2 text-2xl font-semibold text-black font-merriweather">Tidak Ada Profil Guru</h2>
                                <p class="text-gray-600 font-jakarta">Saat ini belum ada sambutan profil guru yang
                                    tersedia.
                ...
                <div class="md:w-[28%] p-5 md:border-l-2 md:border-greenMain md:border-opacity-60">
                    ...
                    <ul wire:scroll class="mt-4 space-y-4">
                        @forelse ($teachers as $teacher)
                            <li wire:key='{{ $teacher->id }}'>
                                <a wire:navigate.hover href="/teacher/{{ $teacher->slug }}" class="lg:w-1/3 sm:w-1/2">
                                    <div class="relative flex">
                                        <img alt="{{ $teacher->image }}" class="absolute inset-0 object-cover object-center w-full h-full"
                                        ...
                                        <div
                                            ...
                                            <h2
                                                ...
                                                {{ $teacher->title }}</h2>
                                            <span
                                                class="text-sm leading-relaxed break-all font-jakarta line-clamp-3">{!! $teacher->description !!}</span>
                                            <div class="flex items-center mt-4">
                                                ...
                                                <span class="ml-2 text-xs text-gray-500 font-jakarta">{{ $teacher->created_at }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </a>
                        ...
                        @empty
                            <div
                                ...
                                <div class="text-center">
                                    ...
                                    <p class="mt-1 text-sm text-center text-gray-600 font-jakarta">Saat ini belum ada profil guru yang
                                        tersedia. Silakan
                    ...
                    @if ($all_teachers->count() > 0)
                        <div class="flex justify-center mt-4 text-sm font-jakarta">
                            <p>Menampilkan <span class="font-semibold">{{ $teachers->count() }}</span> item
                                dari
                                <span class="font-semibold">{{ $all_teachers->count() }}</span> hasil
                            </p>
                        </div>
                    @endif
                    @if ($all_teachers->count() > $loadTeachers && $loadTeachers < $all_teachers->count())
                        <div class="flex justify-center w-full mt-4">
                            <button wire:click='loadMoreTeachers' wire:loading.attr='disabled'
