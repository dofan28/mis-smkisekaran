...
<div>
    ...
    <x-breadcrumb :breadcrumbs="[
        ...
        [
            'name' => 'Detail Halaman PPDB',
            'url' => '/admin/pages/ppdb/' . $pagePPDB->slug,
        ],
    ]"></x-breadcrumb>
    <main class="py-10">
        <div x-data="{ zoomed: false, mouseX: 0, mouseY: 0, imageWidth: 0, imageHeight: 0, showModal: false, modalImage: '' }" x-on:mousemove="mouseX = $event.clientX; mouseY = $event.clientY"
            ...
            <div class="flex mt-2 lg:flex-row md:flex-row sm:flex-col flex-col">
                ...
                <div class="relative mb-4 overflow-hidden lg:w-1/3 lg:mb-0" x-on:mouseleave="zoomed = false"
                    ...
                    <img src="{{ asset('storage/' . $pagePPDB->image) }}" alt="{{ $pagePPDB->title }}"
                        ...
                        x-on:click="
                showModal = true;
                modalImage = '{{ asset('storage/' . $pagePPDB->image) }}';
                ">
                </div>
                <div class="lg:w-2/3 lg:pl-6">
                    <dl class="-my-3 divide-y divide-gray-100 ">
                        ...
                        <div class="grid grid-cols-1 gap-1 py-2 sm:grid-cols-3 sm:gap-4">
                            ...
                            <dd class="text-gray-700 break-words sm:col-span-2 font-jakarta"> {{ $pagePPDB->slug }}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-2 sm:grid-cols-3 sm:gap-4">
                            <dt class="text-sm text-gray-500 uppercase font-jakarta">Kategori</dt>
                            <dd class="text-gray-700 break-words sm:col-span-2 font-jakarta"> {{ $pagePPDB->category }}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-2 sm:grid-cols-3 sm:gap-4">
                            <dt class="text-sm text-gray-500 uppercase font-jakarta">Jenis</dt>
                            <dd class="text-gray-700 sm:col-span-2 font-jakarta"> {{ $pagePPDB->type }}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-2 sm:grid-cols-3 sm:gap-4">
                            <dt class="text-sm text-gray-500 uppercase font-jakarta">Dibuat Pada</dt>
                            <dd class="text-gray-700 sm:col-span-2 font-jakarta">{{ $pagePPDB->created_at->format('d F Y') }}
                                ({{ $pagePPDB->created_at->locale('id')->diffForHumans() }})</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-2 sm:grid-cols-3 sm:gap-4">
                            <dt class="text-sm text-gray-500 uppercase font-jakarta">Terakhir Diperbaruhi</dt>
                            <dd class="text-gray-700 sm:col-span-2 font-jakarta">{{ $pagePPDB->updated_at->format('d F Y') }}
                                ({{ $pagePPDB->updated_at->locale('id')->diffForHumans() }})</dd>
                        </div>
