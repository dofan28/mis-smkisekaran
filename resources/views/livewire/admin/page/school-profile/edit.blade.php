<div>
    ...
    <x-breadcrumb :breadcrumbs="[
        ...
        [
            'name' => 'Edit Halaman Profil Sekolah',
            'url' => '/admin/pages/schoolProfile/' . $pageSchoolProfile->slug . '/edit',
        ],
    ]"></x-breadcrumb>
            ...
            <div class="flex items-center justify-center ">
                <div class="w-full mx-auto bg-white">
                    <form wire:submit='update'>
                        <div class="mb-5">
                            ...
                            <div class="relative">
                                ...
                                <span class="absolute text-xs text-gray-500 right-3 font-jakarta bottom-3">
                                    <span x-text="$wire.title ? $wire.title.length : 0"></span>/55
                                </span>
                            </div>
                            @error('title')
                        ...
                        <div class="mb-5">
                            <label for="type" class="block mb-1 font-semibold text-gray-800 font-jakarta">
                                Jenis
                            </label>
                            <select wire:model='type' name="type" id="type" placeholder="Masukkan jenis"
                                class="w-full rounded-md border  appearance-none bg-white py-3 px-5 text-sm  text-gray-600 outline-none focus:border-greenMain font-jakarta focus:shadow-md @error('type') border-red-500 @enderror">
                                <option value="">Pilih Jenis</option>
                                <option value="intro">Sambutan Kepala Sekolah</option>
                                <option value="history">Sejarah Sekolah</option>
                                <option value="visionmission">Visi dan Misi</option>
                                <option value="organizationalstructure">Struktur Organisasi</option>
                                <option value="documents">Dokumen</option>
                                <option value="lecture">Guru dan Karyawan</option>
                                <option value="schoolprogram">Program Sekolah</option>
                                <option value="schoolachievement">Prestasi Sekolah</option>
                            </select>
                            @error('type')
                                <p class="text-xs italic text-red-500 font-jakarta">{{ $message }}</p>
                            @enderror
                            ...
                            <div x-data="{ dragging: false, fileName: '', fileSize: 0 }" class="relative w-full">
                                <div class="items-center justify-center mx-auto">
                                    <label :class="{ 'border-greenMain': dragging, 'border-gray-300': !dragging }"
                                        ...
                                        class="flex sm:flex-row flex-col @if (!$newImage && !$image) justify-center @endif w-full sm:h-24 px-3 transition bg-white border-2 border-dashed rounded-md appearance-none cursor-pointer hover:border-gray-600 focus:outline-none space-x-4"
                                        id="drop">
                                        @if (!$newImage && !$image)
                                            <span wire:loading.remove wire:target="newImage"
                                                ...
                                                <span class="text-sm font-medium text-gray-600 font-jakarta">Seret file
                                        ...
                                        <div class="flex flex-col justify-center">
                                            <input wire:model="newImage" type="file" name="newImage"
                                                x-ref="fileInput"
                                                @change="fileName = $refs.fileInput.files[0].name; fileSize = ($refs.fileInput.files[0].size / 1024).toFixed(2)"
                                                class="font-jakarta file:hidden @if (!$newImage) hidden @endif @error('image') border-red-500 @enderror"
                                                accept="image/*" id="image">

                                            @if ($newImage || $image)
                                                <template x-if="fileName">
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold text-gray-600 font-jakarta">Nama
                                                            File: <span x-text="fileName"></span></p>
                                                        <p class="text-sm text-gray-600 font-jakarta">Ukuran File: <span
                                                                x-text="fileSize"></span> KB</p>
                                                    </div>
                                                </template>
                                        ...
                                        <div class="flex flex-col items-center justify-center">
                                            <div wire:loading wire:target="newImage" aria-label="Mengunggah..."
                                            ...
                                            <button wire:loading wire:target='newImage' type="button"
                                                wire:click="$cancelUpload('newImage')"
                        ...
                        <div>
                            <button wire:loading.class='bg-amber-700' wire:loading.attr='disabled' type="submit"
                                ...
                                <span wire:loading.remove>Edit Halaman Profil Sekolah</span>
