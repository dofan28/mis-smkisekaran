<div>
    ...
    <x-breadcrumb :breadcrumbs="[
        ...
        [
            'name' => 'Mengelola Halaman Kompetensi Keahlian',
            'url' => '/admin/pages/competencySkill',
        ],
    ]"></x-breadcrumb>
    ...
    <main class="py-10">
        <div class="p-6 bg-white rounded-lg shadow-lg">
            <a wire:navigate href="/admin/pages/competencySkill/create"
                class="flex items-center w-60 px-3 py-2 space-x-2 font-semibold text-white transition duration-300 ease-in-out transform rounded-lg shadow-lg bg-amber-500 hover:bg-amber-700 hover:scale-105">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                </svg>
                <span class="text-sm font-jakarta">Buat Kompetensi Keahlian</span>
            </a>
            <div class="mt-5 overflow-x-auto">
                @if ($pageCompetencySkills->isNotEmpty())
                    <!-- Desktop version -->
                    <table class="hidden min-w-full bg-white border-separate border-spacing-0 sm:table">
                        ...
                        <tbody class="text-gray-800">
                            @foreach ($pageCompetencySkills as $pageCompetencySkill)
                                <tr wire:key='{{ $pageCompetencySkill->id }}'
                                    ...
                                    <td class="px-4 py-3 border border-gray-100">
                                        <div class="break-words line-clamp-2">
                                            {{ $pageCompetencySkill->title }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 border border-gray-100">
                                        <div class="break-words line-clamp-2">
                                            {{ $pageCompetencySkill->type }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 border border-gray-100">
                                        <div class="break-words line-clamp-2">
                                            {!! $pageCompetencySkill->content !!}
                                        </div>
                                    </td>
                                    <td class="flex justify-center px-4 py-3 border border-gray-100">
                                        <img src="{{ asset('storage/' . $pageCompetencySkill->image) }}"
                                            class="object-cover object-center h-24 border-2 border-gray-400 rounded-md shadow-md"
                                            alt="{{ $pageCompetencySkill->title }}">
                                    </td>
                                    <td class="px-4 py-3 border border-gray-100">
                                        <div class="inline-flex items-center rounded-md shadow-sm">
                                            <a wire:navigate
                                                href="/admin/pages/competencySkill/{{ $pageCompetencySkill->slug }}/edit">
                                                <button
                                            ...
                                            <a wire:navigate
                                                href="/admin/pages/competencySkill/{{ $pageCompetencySkill->slug }}">
                                                <button
                                            ...
                                            <x-modal-confirmation action="delete" :identify="'halaman kompetensi keahlian ' .
                                                $pageCompetencySkill->title .
                                                ''" :data="$pageCompetencySkill">
                    ...
                    <div class="sm:hidden">
                        @foreach ($pageCompetencySkills as $pageCompetencySkill)
                            <div class="mb-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                                <div class="p-4">
                                    <h3 class="mb-2 text-lg line-clamp-2 font-semibold text-gray-800">
                                        {{ $pageCompetencySkill->title }}</h3>
                                    ...
                                    <div class="flex items-center justify-center mt-3">
                                        ...
                                        <div class="flex space-x-2">
                                            ...
                                            <a wire:navigate href="/admin/pages/competencySkill/{{ $pageCompetencySkill->slug }}">
                                                <button
                                            ...
                                            <x-modal-confirmation action="delete" :identify="'pengumuman ' . $pageCompetencySkill->title . ''" :data="$pageCompetencySkill">
                                                <button @click="showModal = true"
                ...
                @else
                    <div class="text-center text-gray-800 font-jakarta">
                        <h3>Data halaman kompetensi keahlian tidak tersedia. Silakan buat <a wire:navigate
                                href="/admin/pages/competencySkill/create" class="underline text-greenMain">halaman
                                kompetensi keahlian baru</a>.</h3>
                    </div>
                @endif
            </div>
        </div>
        <x-pagination :items="$pageCompetencySkills" />
    </main>
</div>
