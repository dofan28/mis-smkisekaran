...
<body x-data="{ scrolledFromTop: false }" x-init="window.pageYOffset >= 50 ? scrolledFromTop = true : scrolledFromTop = false"
    ...
    <div class="flex flex-col-reverse items-center justify-center w-screen h-screen  bg-gray-100 sm:flex-row">
        <aside x-data="{ active: window.location.pathname }"
            class="flex items-center justify-around w-full h-16 flex-shrink-0 overflow-hidden  bg-white shadow-lg sm:flex-col sm:pt-4 sm:w-24 sm:h-full">
            @include('components.layouts._dashboard_sidebar')
        </aside>
        <div class="flex flex-col flex-grow sm:h-full h-0 w-full overflow-auto p-10">
            {{ $slot }}
        </div>
    </div>
